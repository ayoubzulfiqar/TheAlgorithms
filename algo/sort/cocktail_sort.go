...
The sorting process is performed inside a loop that continues as long as swaps are being made during a pass.
...
In the forward pass, the code iterates through the unsorted portion of the array from `start` to `end-1`.
It compares adjacent elements and swaps them if they are out of order (i.e., if `array[i]` is greater than `array[i+1]`).
...
After the forward pass, if no swaps were made (`swapped` is `false`), it means that the array is already
...
reverse order (from `end-1` down to `start`). It compares adjacent elements and swaps them if they are out of order.
...
After the backward pass, the `start` pointer is increased by 1 because the smallest element is now in
...
Finally, when the loop exits (indicating that the entire array is sorted), the function returns the sorted `array`.
