...
func TestStrandFloatSort(t *testing.T) {
	...
	sortedArray := sorts.Cycle(unsortedArray)
...
func BenchmarkINTStrandSort(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		b.StartTimer()
		sorts.Strand(arrayCopy)

		b.StopTimer()
	}
}

func BenchmarkFLOATStrandSort(b *testing.B) {
	// Generate a random unsorted array for each benchmark iteration
